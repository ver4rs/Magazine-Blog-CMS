...
if (mysql_num_rows($over1) == FALSE) {
  ...
  $zapis = 'INSERT INTO pocitadlo (pocitadlo_id, pocitadlo_ip, pocitadlo_navstev, pocitadlo_ip_pocet, pocitadlo_celkovo, pocitadlo_datum)
            VALUES (NULL,
                    "' . mysql_real_escape_string($ipAdresa) . '",
                    "' . mysql_real_escape_string($pocet_navstev) . '",
                    "' . mysql_real_escape_string($pocet_navstev) . '",
                    "' . mysql_real_escape_string($pocet_navstev) . '",
...
$zapis1 = mysql_query($zapis)or die(mysql_error());
  ...
  $nacitaj10 = mysql_query($nacitaj)or die(mysql_error());

  if (mysql_num_rows($nacitaj10) != '0') {
    $riadok = mysql_fetch_array($nacitaj10);
    //cas nastaveny +45minnut na odstranenie starych zaznamov
    $datum_a = strtotime($riadok['pocitadlo_datum']);
    $datum_zmena = $datum_a +(60*45);
    ...
    if (date('Y-m-d H:i:s') > $datum ) {
      ...
      $prikaz = 'UPDATE pocitadlo SET
                ...
                WHERE pocitadlo_ip = "' . mysql_real_escape_string($ipAdresa) . '"';
